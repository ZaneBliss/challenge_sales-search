    ...
    salesByWeek.forEach(sale => {
      for (const value of Object.values(sale.sales_agent)) {
        if (value.toLowerCase().includes(searchTerm)) {
          app.innerHTML += `<h1>${sale.sales_agent.first_name} ${sale.sales_agent.last_name}</h1>`
          app.innerHTML += `
          <p>${sale.sales_agent.mobile}</p>
          <p>${sale.sales_agent.email}</p>
          <p>Sale: $${sale.gross_profit}</p>
          `
        }
      }
    });
